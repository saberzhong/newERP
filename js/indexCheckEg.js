    ...
    loginCheck: function () {
        ...
        var userCheck = function userCheck(_this1) {
            if ($(_this1).val().search(/\s+/) === 0 || $(_this1).val() === '') {
                $('.errorInfo').empty().append('Cannot be empty').css('color', 'orange');
                ok1 = false;
            } else {
                ok1 = true;
            }
        ...
        var pswCheck = function pswCheck(_this2) {
            if ($(_this2).val().search(/\s+/) === 0 || $(_this2).val() === '') {
                $('.errorInfo').empty().append('Cannot be blank').css('color', 'orange');
                ok2 = false;
            } else {
                $('.errorInfo').empty();
                ok2 = true;
        ...
        var codeCheck = function codeCheck(_this4) {
            if ($(_this4).val().search(/\s+/) === 0 || $(_this4).val() === '') {
                $('.errorInfo').empty().append('verification code must be filled').css('color', 'orange');
                ok4 = false;
            } else {
                $('.errorInfo').empty();
                ok4 = true;
            ...
            if (ok1 && ok2 && ok3 && ok4) {
                ...
                $.post('loginAction!login.action', {
                    ...
                    if (eval(data.number) > eval(2)) {
                        alert('Your password error more than 3 times, please re-login page');
                    }
                    if (typeof (data.location) !== 'undefined' && data.location !== 'indexeg.html') {
            ...
            if (ok1 && ok2) {
                ...
                if (username && password) {
                    $.post('superAdminAction!superAdminLogin.action', {
                        ...
                        if (res.code === 1) {
                            location.assign('pages/superuser/superEg.html');
                        } else {
    ...
    createGroup: function () {
        ...
        var groupname = function groupname(_this) {
            ...
            if ($(_this).val().search(/\s+/) === 0 || $(_this).val() === '') {
                ...
                $('.error').empty().append(str + 'Cannot be empty').css('color', 'orange');
                ok1 = false;
            } else {
                // 不为空时，取得输入组名和已存在组名判断
                $.get('gameGroupAction!isGameGroupExist.action?rnd=' + Math.random(), {
                    ...
                    if (data === true) {
                        ...
                        $('.error').empty().append('The group already exists');
                        ok2 = false;
                        return false;
                    }
                }, 'json');
        ...
        var creategroupCheck = function creategroupCheck(_this) {
            ...
            if ($(_this).val().search(/\s+/) === 0 || $(_this).val() === '') {
                ...
                $('.error').empty().append('The specified range is(0-50)').css('color', 'orange');
                ok1 = false;
            } else if ($(_this).val().match(/^[0-9]*[1-9][0-9]*$/) === null) {
                $('.error').empty().append('Please enter correct format').css('color', 'orange');
                ok1 = false;
            } else {
                $(_this).next()
                    .empty()
        ...
        $('input[name="create"]').click(function () {
            ...
            if (ok1 && ok2) {
                ...
                alert('Please re-enter the group name');
            } else {
