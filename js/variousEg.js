...
$(function () {
    ...
    function ReadyMarket(data) {
        ...
        $(_firstMarket).each(function (i, items) {
            ...
            switch(items.marketName)
            {
                case "本地市场":items.marketName="Local Market";break;
                case "国内市场":items.marketName="Domestic Market";break;
                case "区域市场":items.marketName="Regional Market";break;
                case "国际市场":items.marketName="International Market";break;
                case "亚洲市场":items.marketName="Asia Market";break;
            }
            readyH2.eq(0).text(items.marketName);
            readyH2.eq(1).text(items.maintainCost);

            if (items.status === 1) {
                readyH2.eq(2).text('maintenance');
            } else if (items.status === 0) {
                readyH2.eq(2).text('Pause');
            } else {
                return false;
            }

            if (items.lastStatus === 1) {
                readyH2.eq(3).text('The last period keep it up');
            }	else if (items.lastStatus === 0) {
                readyH2.eq(3).text('Not maintained');
            }	else {
                return false;
            }
        });
    ...
    function Update(Open, items) {
        ...
        if (items.status === 1) {
            openHh2.eq(5).text('developing');
            Open.find('h2:last').text('Pause');

        }	else if (items.status === 0) {
            openHh2.eq(5).text('Suspension');
            Open.find('h2:last').text('Continue');
        } else {
            return false;
        }
    }
    ...
    $(document).on('click', '.notOpen', function () {
        ...
        $.post('marketAction!startUndevelopMarketToDeveloping.action', {
            'marketName': toEnglishMarket(marketName)
        }, function (data) {
            removeMarket.remove();
    ...
    $(document).on('click', '.readyOpenBtn', function () {
        ...
        if (_h2.text() === 'Pause') {
            _h2.text('Start maintenance');
            prevH2.eq(2).text('Pause');
            Submit('marketAction!stopDevelopedMarket.action', prevH2);
        } else {
            _h2.text('Pause');
            prevH2.eq(2).text('maintenance');
    ...
    $(document).on('click', '.openBtn', function () {
        ...
        if (_hh2.text() === 'Suspension') {
            _hh2.text('Continue');
            prevHh2.eq(5).text('Suspension');
            Submit('marketAction!stopDevelopingMarket.action', prevHh2);
        } else {
            _hh2.text('Suspension');
            prevHh2.eq(5).text('Continue');
